...
public class MeterTypeService
{
    ...
    private List<string> _defaultManufacturers = new();
    ...
    public async Task<List<string>> GetManufacturersAsync(string search, int? take = null)
    {
        if (string.IsNullOrWhiteSpace(search))
        {
        ...
        var url = "/api/metertypes/manufacturers";
        if (!string.IsNullOrWhiteSpace(search) || take.HasValue)
        {
            var query = new List<string>();
            if (!string.IsNullOrWhiteSpace(search))
...
public record MeterTypeDto(int Id, string ManufacturerName, string Type, string FullName);
